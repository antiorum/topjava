...
- 6: **Проверьте выполение ВСЕХ тестов через maven**. В случае проблем проверьте, что не портите константу из `ru.javawebinar.topjava.MealTestData`
...
- 9: Посмотрите, нет ли в `ru.javawebinar.topjava.MealTestData` методов, которые можно сделать общими (через generic и `TestUtil`)
